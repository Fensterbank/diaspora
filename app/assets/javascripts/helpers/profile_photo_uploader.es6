...
Diaspora.ProfilePhotoUploader = class {
  ...
  constructor() {
     ...
     */
    this.showMessage = (type, text) => { return app.flashMessages ? app.flashMessages[type](text) : alert(text); };
  ...
  initFineUploader() {
    this.fineUploader = new qq.FineUploaderBasic({
      ...
      callbacks: {
        onProgress:
          (id, fileName, loaded, total) => (this.info.innerText = `${fileName} ${Math.round(loaded / total * 100)}%`),
  ...
  initCropper(imageData) {
    ...
    this.picture.onload = () => {
      ...
      this.picture.setAttribute("style", (this.picture.naturalHeight > this.picture.naturalWidth ?
        "max-height:600px;max-width:none;" : "max-width:600px;"));
      this.buildControls();
  ...
  buildControls() {
    ...
    this.controlRow.append($("<div class=\"btn-group\" role=\"group\">")
      .append(this.controls.rotateLeft).append(this.controls.rotateRight));

    // preview images in the middle
    this.controlRow.append($("<div class=\"previews\">")
      .append("<div class=\"preview-lg\">")
      .append("<div class=\"preview-md\">")
      .append("<div class=\"preview-sm\">"));

    // main buttons on the right
    this.controlRow.append($("<div class=\"btn-group\" role=\"group\">")
      .append(this.controls.reset).append(this.controls.cancel).append(this.controls.accept));
  }
