...
Diaspora.ProfilePhotoUploader = class {
  ...
  constructor() {
    console.log('ProfilePhotoUploader constructed!');
    ...
    this.createButton = (icon, onClick) =>
      ($(`<button class="btn btn-default" type="button"><i class="entypo-${icon}"></i></button>`).on('click', onClick));
  ...
  onUploadCompleted(id, fileName, responseJSON) {
    ...
    this.fileInfo.innerText = Diaspora.I18n.t("photo_uploader.completed", {"file": fileName});

    if (responseJSON.data !== undefined) {
      /* flash message prompt */
      const message = Diaspora.I18n.t("photo_uploader.looking_good");
      (app.flashMessages ? app.flashMessages.success(message) : alert(message));
  ...
  initFineUploader() {
    console.log("initializeFineUploader");
    this.fineUploader = new qq.FineUploaderBasic({
      ...
      callbacks: {
        onProgress: (id, fileName, loaded, total) => (this.fileInfo.innerText = `${fileName} ${Math.round(loaded / total * 100)}%`),
        ...
        onError: (id, name, errorReason) => (app.flashMessages ? app.flashMessages.error(errorReason) : alert(errorReason))
      }
    });
  }
  ...
  cropImage() {
    ...
    // if the selected crop box is bigger than our defined maximum size, it will be rescaled
    // it's needed to prevent upload errors when cropping giant pictures.
    const canvasOptions = (data.width > maxSize || data.height > maxSize) ? { width: maxSize, height: maxSize } : null;
  ...
  buildControls() {
    ...
    this.controlRow.append($('<div class="btn-group" role="group">').append(this.controls.rotateLeft).append(this.controls.rotateRight));
    this.controlRow.append($('<div class="btn-group" role="group">').append(this.controls.reset).append(this.controls.cancel).append(this.controls.accept));
  }
  ...
  initCropper(imageData) {
    ...
    this.picture.onload = () => {
      // set style based on picture orientation
      this.picture.setAttribute('style', (this.picture.naturalHeight > this.picture.naturalWidth ? 'max-height:600px;max-width:none;' : 'max-width:600px;'));
      this.buildControls();

      this.setLoading(false);

      console.log(this.picture.naturalWidth + 'x' + this.picture.naturalHeight);
